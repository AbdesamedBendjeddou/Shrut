...
use gloo::{console::{error, log}, file::Blob};
use js_sys::{Array, Object, Reflect, Uint8Array, ArrayBuffer};
...
use crate::components::atoms::{
    ...
    other_peers_state::WebRTCRole, avatar::FileData,
};
...
impl WebRtcConnection {
    ...
    fn set_peeer_connection_on_data_channel(&self, other_peer: Uuid) {
                ...
                let onmessage_callback =
                    Closure::wrap(
                        ...
                        let message = ev.data().dyn_into::<ArrayBuffer>().ok();
                            log!("message recv fonm on data channel", format!("{:?}",message.clone()));
                            if let Some(message) = message{
                                
                                    log!("message from datachannel (will call on_message)", format!("{:?}", message.byte_length()));
        ...
        self.peer_connection.set_ondatachannel(Some(on_datachannel.as_ref().unchecked_ref()));
    ...
    fn set_on_open(&self) {
        ...
        self.data_channel.set_onopen(Some(on_open_callback.as_ref().unchecked_ref()));
    ...
    fn set_on_message_callback(&self, other_peer: Uuid) {
        ...
        let on_message_callback: Box<dyn FnMut(MessageEvent)> =
            Box::new(move |ev: MessageEvent| {
                ...
                if let Some(message) = message.and_then(|t| rmp_serde::from_slice::<FileData>(&t.to_vec()).ok()) {
                ...
                match ev.data().as_string() {
                    Some(message) => {
                        log!("this peer received: {}", message, "from", other_peer.to_string());
        ...
        self.data_channel.set_onmessage(Some(on_message_callback.as_ref().unchecked_ref()));
    ...
    fn set_on_error_callback(&self) {
        ...
        self.data_channel.set_onerror(Some(on_error.as_ref().unchecked_ref()));
        on_error.forget();
    }
